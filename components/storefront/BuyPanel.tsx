...
export function BuyPanel({ product, variants, attributeOrder, themeButton }: Props) {
  ...
  const isFree = Number(product.price) === 0 && !isPhysical;
  ...
  const slots = useMemo(() => {
    ...
    const availability =
      (config.availability as Record<string, { enabled?: boolean; from?: string; to?: string }>) ?? {};
    ...
    for (let d = 1; d <= 14 && out.length < 40; d++) {
      ...
      const end = new Date(day); end.setHours(th_, tm, 0, 0);
      for (let s = new Date(start); s.getTime() + duration * 60000 <= end.getTime(); s = new Date(s.getTime() + duration * 60000)) {
        out.push({
          iso: s.toISOString(),
  ...
  const ctaLabel =
    product.type === "webinar" ? t("register")
    : isCoaching ? t("book")
    : product.type === "community" ? t("join")
    : isFree ? t("getFree")
    : t("buy");
    ...
    <div>
              ...
              {slots.map((s) => (
                <button
                  ...
                  className={`rounded-xl border px-3 py-2 text-xs font-medium ${
                    slot === s.iso ? "border-transparent text-white" : "border-current opacity-60"
                  }`}
      ...
      <div className="mt-6">
        ...
        >
          ...
          {isFree
            ? t("free")
            : `${(unitPrice * quantity + (isPhysical ? Number(config.shipping_fee ?? 0) : 0)).toLocaleString()} ${product.currency}`}
        </button>
      </div>
...
function CheckoutSheet({
  product, variantId, quantity, slot, unitPrice, onClose, themeButton,
}: {
  ...
  const [ship, setShip] = useState({ name: "", phone: "", address: "", city: "", notes: "", cod: false });
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const shippingFee = isPhysical ? Number(config.shipping_fee ?? 0) : 0;
    ...
    try {
      const res = await fetch("/api/checkout", {
        ...
        body: JSON.stringify({
          ...
          shipping: isPhysical
            ? { name: ship.name, phone: ship.phone, address: ship.address, city: ship.city, notes: ship.notes || undefined, cod: ship.cod }
            : undefined,
        }),
      });
      ...
      if (res.ok && body.orderId) {
        window.location.assign(
          locale === "en" ? `/order/${body.orderId}` : `/${locale}/order/${body.orderId}`
        );
      } else {
        setError(body.error === "out_of_stock" ? t("outOfStock") : t("errGeneric"));
        setBusy(false);
      }
    } catch {
      ...
      >
        ...
        <div className="mt-3 rounded-2xl bg-neutral-50 p-4 text-sm text-neutral-700">
          ...
          <div className="mt-2 space-y-1">
            <Row label={`${t("item")} × ${quantity}`} value={`${itemTotal.toLocaleString()} ${product.currency}`} />
            {shippingFee > 0 && (
              <Row label={t("shipping")} value={`${shippingFee.toLocaleString()} ${product.currency}`} />
            )}
            <div className="border-t border-neutral-200 pt-1">
              <Row bold label={t("total")} value={isFree ? t("free") : `${total.toLocaleString()} ${product.currency}`} />
            </div>
          </div>
        </div>
        ...
        <form onSubmit={placeOrder} className="mt-4 space-y-3">
          ...
          <input type="email" required placeholder={t("yourEmail")} value={email}
            onChange={(e) => setEmail(e.target.value)} className={input} />
          <input placeholder={t("yourName")} value={name} maxLength={80}
          ...
          {isCustom && (
            <textarea
              ...
              value={answer} onChange={(e) => setAnswer(e.target.value)} className={input}
            />
          )}
            ...
            <>
              ...
              {Boolean(config.cod_enabled) && (
                <label className="flex items-center gap-2.5 text-sm font-medium text-neutral-800">
                  <input type="checkbox" checked={ship.cod}
                    onChange={(e) => setShip({ ...ship, cod: e.target.checked })} className="h-4 w-4" />
                  {t("cod")}
                </label>
              )}
            </>
          ...
          <p className="text-center text-xs text-neutral-500">{t("noPayNote")}</p>
        </form>
      </div>
    </div>
  );
